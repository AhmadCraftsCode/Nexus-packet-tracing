...
class Rider { public: string name; int load; int capacity; Rider() : name(""), load(0), capacity(5) {} Rider(const string& n, int cap) : name(n), load(0), capacity(cap) {} };
                    ...
                    public:
                        ...
                        CourierSystem() : byId(8192), pq(16384), routing(graph), serialCounter(0), riderCount(4) {
                            // 1. Initialize System Personnel (Riders/Handlers)
                            riders[0] = Rider("Alpha-Node-Handler", 10);
                            riders[1] = Rider("Beta-Link-Processor", 8);
                            riders[2] = Rider("Gamma-Carrier", 5);
                            riders[3] = Rider("Delta-Router", 5);

                            // 2. Hardcoded Network Topology (The Map)
                            graph.addUndirectedRoad("Lahore", "Islamabad", 380, 5);
                            graph.addUndirectedRoad("Lahore", "Karachi", 1240, 15);
                            graph.addUndirectedRoad("Lahore", "Faisalabad", 180, 2);
                            graph.addUndirectedRoad("Islamabad", "Peshawar", 150, 8);
                            graph.addUndirectedRoad("Karachi", "Quetta", 700, 10);
                            graph.addUndirectedRoad("Faisalabad", "Multan", 240, 5);
                            graph.addUndirectedRoad("Multan", "Karachi", 900, 12);

                            // 3. HARDCODED INITIAL PACKETS (So it's not empty)
                            // Format: ID, Sender, Receiver, Source, Dest, Weight, Priority, Serial
                            Parcel* p1 = new Parcel("9901", "Core-Server", "User-Node-A", "Lahore", "Islamabad", 0.5, PRIORITY_OVERNIGHT, serialCounter++);
                            addParcelInternal(p1);

                            Parcel* p2 = new Parcel("9902", "DB-Cluster", "Client-X", "Karachi", "Lahore", 1.2, PRIORITY_NORMAL, serialCounter++);
                            addParcelInternal(p2);

                            logs.add("Network System initialized with 7 backbone links.");
                        }       
                        ...
                        void operationsCLI() {
                            while (true) {
                                cout << "\nCourier Operations:\n";
                                cout << "1) Move from Warehouse -> Transit (Load)\n";
                                cout << "2) Assign Riders (priority-based, load-aware)\n";
                    ...
                    static void printMenu() {
                        printHeader("NEXUS PACKET TRACING & ROUTING ENGINE");
                        cout << " [1]  Inject New Data Packet" << endl;
                        cout << " [2]  Purge Packet from Buffer" << endl;
                        cout << " [3]  View Sorted Routing Queue" << endl;
                        cout << " [4]  Toggle Node/Link Blockage" << endl;
                        cout << " [5]  Dispatch Buffer to Warehouse" << endl;
                        cout << " [6]  Trace Packet Real-Time" << endl;
                        cout << " [7]  View Packet Hop-History" << endl;
                        cout << " [8]  Network Operations (Simulate)" << endl;
                        cout << " [9]  Undo Last System Action" << endl;
                        cout << " [10] Replay System Logs" << endl;
                    ...
                    int main() {
                        ...
                        clearScreen();
                        cout << "Booting Nexus Engine..." << endl;
                        delay(1000);
                        ...
                        while (true) {
                            ...
                            if (ch == 0) {
                                cout << "Deactivating links... Goodbye." << endl;
                                delay(800);
                                return 0;
                            }

                            // Add clearScreen before each action
                            clearScreen();
                            ...
                            default: cout << "Link Error: Unknown Command." << endl; break;
                            }
